...
window.onload = function () {
    hui.define('account_read', ['hui@0.0.1', 'hui_requester@0.0.1', 'hui_md5@0.0.1', 'hui_action@0.0.1', 'hui_textinput@0.0.1', 'hui_button@0.0.1', 'hui_checkbox@0.0.1'], function () {
        hui.Validator.setRule('email', {
            'validate': function (text) {
                ...
                else if (!/^([^@\s]+)@((?:[-a-zA-Z0-9]+\.)+[a-zA-Z]{2,})$/ig.test(text)) {
                    return 3;
                }
                var me = this;
        ...
        hui.Router.setRule('/read', 'AccountRead');

        hui.AccountRead = function (options, pending) {
            hui.AccountRead.superClass.call(this, options, 'pending');

            this.type = 'account_read';
            this.controlMap = {};
        ...
        hui.AccountRead.prototype = {
            getView: function () {
                return '<span ui="type:\'Label\',formName:\'UserName\',value:\'#{UserName}\'">课程名称</span><button ui="type:\'Button\',formName:\'Submit\'" class="hj001_login_btn" tabindex="5">修改</button>';
            },
            initModel: function (callback) {
                var me = this;
                hui.Mockup.set('/api/get_userinfo', {
                    code: 0,
                    message: '',
                    data: 'Tom'
                });
                window.Requester.JSONP('/api/get_userinfo?uid=1001', {
                    data: {},
                    onsuccess: function (result) {
                        if (String(result.code) === '-1') {
                            ...
                            me.model.set('UserName', result.data);
                        }
                        callback && callback();
                    }
                });
            ...
            render: function (options) {
                hui.AccountRead.superClass.prototype.render.call(this);
                //var me = this;
                // 渲染对话框
            ...
            initBehavior: function (controlMap) {
                var me = this;
                //hui.Action.get().setValueByTree({'UserName': 'wwww'})
            ...
            onsubmit: function () {
                //var me = this;
                hui.Locator.redirect('/edit');
            }

        };
        /*通过hui.Control派生hui.Button*/
        // hui.Control.derive(hui.AccountRead);
        /* hui.AccountRead 继承了 hui.Control */
        hui.inherits(hui.AccountRead, hui.Action);
    });
    hui.define('account_edit', ['hui@0.0.1', 'hui_requester@0.0.1', 'hui_md5@0.0.1', 'hui_action@0.0.1', 'hui_textinput@0.0.1', 'hui_button@0.0.1', 'hui_checkbox@0.0.1'], function () {
        hui.Router.setRule('/edit', 'AccountEdit');

        hui.AccountEdit = function (options, pending) {
            hui.AccountEdit.superClass.call(this, options, 'pending');

            this.type = 'account_edit';
            this.controlMap = {};
        ...
        hui.AccountEdit.prototype = {
            getView: function () {
                return '<input type="text" ui="type:\'TextInput\',formName:\'UserName\',rule:\'email\',autoHideError:true,allowSpace:false,hideBlankError:true,DBC2SBC:true" class="hj001_ipt" maxlength="50"  tabindex="1" style="ime-mode: disabled" /><button ui="type:\'Button\',formName:\'Submit\'" class="hj001_login_btn" tabindex="5">保存</button>';
            },
            render: function (options) {
                hui.AccountEdit.superClass.prototype.render.call(this);
                //var me = this;
                // 渲染对话框
                //hui.Control.init(me.getMain(), (hui.Action ? hui.Action.get() : {}).model, me);
            },
            ...
            submitUserInfo: function () {
                var me = this,
                    param = me.getParamMap();
                if (me.validate()) {
                    hui.Mockup.set('/api/save_userinfo', {
                        code: 0,
                        message: '',
                        data: param.UserName
                    });
                    window.Requester.JSONP('/api/save_userinfo', {
                        data: param,
                        onsuccess: function (result) {
                            if (result.code === 0) {
                                ...
                                hui.Validator.showError(hui.c('hj001_login_btn')[0], result.message);
                            }
                        }
                    });
                }
        ...
        /* hui.AccountEdit 继承了 hui.Control */
        hui.inherits(hui.AccountEdit, hui.Action);
